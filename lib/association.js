...
class Association extends EventEmitter {
  ...
  send(buffer, options, callback) {
    ...
    if (streamId < 0 || streamId > this.OS) {
      this.debugger.warn('wrong stream id %d', streamId)
      return callback('wrong stream id ' + streamId)
    }
    ...
    if (this.state === 'SHUTDOWN-PENDING' || this.state === 'SHUTDOWN-RECEIVED') {
       ...
       */
      return callback('not accepting new data in SHUTDOWN state')
    }
    ...
    */
    ...
    if (buffer.length >= this.peer_rwnd) {
      return callback('peer has no buffer space (rwnd) for new packet ' + this.peer_rwnd)
    }
    ...
    */
    if (this.flightsize >= this.cwnd) {
      return callback('flightsize >= cwnd ' + this.flightsize + ' ' + this.cwnd)
    }
